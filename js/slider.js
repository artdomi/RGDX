function Slider(obj, callback, ctrl, layerStatic, layerDynamic, parentDivID) {
    ...
    var text = {x: 0, y: 0, xRead: 0, yRead: ctrl.h/2, w: displayW, h: ctrl.h/2,
                xInfo: ctrl.x + textCentered*displayW/2, yInfo: ctrl.y-2*ctrl.h/5, wInfo: ctrl.w, hInfo: 2*ctrl.h/6, colour: '#b4b4b4', margin: '-30px 0 0 0'};
    ...
    var input1 = {id: sliderID + "Input", value: displayValue.in, class: "sliderTextInput", w: text.w, h: ((readout == 0) ? ctrl.h : text.h), fontSize: text.h-5, colour: "black"};
    ...
    switch (ctrl.axis){
        case 'linear':  ctrl.transform = function(param){return param}; // FUNCTIONAL COORD DEPENDENCE ON POSITION
                        ...
                        ctrl.ticksMax = getMod(slider.w, 60);
    ...
    function updateSlider() {
        ...
        $('#' + sliderID + "Input").val(Math.round(displayValue.in));
    }
...
// $( "#graph" ).draggable();
    ...
    $('#' + sliderID + 'Input').change(function (e) {
        ...
        updateSlider();
        e.target.value = Math.round(displayValue.in);
